...
mergePSets <- function(mDataPSet, sensDataPSet, commonCellsOnly=FALSE, ...){
  ...
  #acell <- setdiff(rownames(cellInfo(mDataPSet)), rownames(cellInfo(sensDataPSet)))
	...
	cell.info.df[setdiff(rownames(cellInfo(sensDataPSet)), rownames(cellInfo(mDataPSet))), "dataset"] <- pSetName(sensDataPSet)
	cell.info.df[setdiff(rownames(cellInfo(mDataPSet)), rownames(cellInfo(sensDataPSet))), "dataset"] <- pSetName(mDataPSet)
	cell.info.df[intersect(rownames(cellInfo(mDataPSet)), rownames(cellInfo(sensDataPSet))), "dataset"] <- paste0(pSetName(sensDataPSet), "///", pSetName(mDataPSet))
	...
	cell.curation.df[setdiff(rownames(sensDataPSet@curation$cell), rownames(mDataPSet@curation$cell)), "dataset"] <- pSetName(sensDataPSet)
	cell.curation.df[setdiff(rownames(mDataPSet@curation$cell), rownames(sensDataPSet@curation$cell)), "dataset"] <- pSetName(mDataPSet)
	cell.curation.df[intersect(rownames(mDataPSet@curation$cell), rownames(sensDataPSet@curation$cell)), "dataset"] <- paste0(pSetName(sensDataPSet), "///", pSetName(mDataPSet))
	...
	tissue.curation.df[setdiff(rownames(sensDataPSet@curation$tissue), rownames(mDataPSet@curation$tissue)), "dataset"] <- pSetName(sensDataPSet)
	tissue.curation.df[setdiff(rownames(mDataPSet@curation$tissue), rownames(sensDataPSet@curation$tissue)), "dataset"] <- pSetName(mDataPSet)
	tissue.curation.df[intersect(rownames(mDataPSet@curation$tissue), rownames(sensDataPSet@curation$tissue)), "dataset"] <- paste0(pSetName(sensDataPSet), "///", pSetName(mDataPSet))
	mergePSet@curation$tissue <- tissue.curation.df

	mergePSet@sensitivity$n <- PharmacoGx:::.summarizeSensitivityNumbers(mergePSet)

	mergePSet@annotation$name <- paste(pSetName(mDataPSet), pSetName(sensDataPSet), sep=".")
...
CCLE <- readRDS("/pfs/getCCLE/CCLE.rds")
CTRPv2 <- readRDS("/pfs/get_CTRP/CTRPv2.rds")

CCLE_CTRPv2_merged <- mergePSets(mDataPSet = CCLE, sensDataPSet = CTRPv2)

saveRDS(CCLE_CTRPv2_merged, file="/pfs/out/CCLE_CTRPv2_merged.rds")
